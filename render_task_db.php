...
function get_task_employee($id){
    $sql = "SELECT `task`.`TASK_ID`, `task`.`TIEU_DE`, `task_info`.`MA_NGUOI_NHAN`, `user`.`HO_TEN`, `task_info`.`STATUS`, `task`.`DEADLINE` FROM `task`, `task_info`, `user`, `user_info` WHERE `task_info`.`STATUS` != 'Completed' AND `task_info`.`STATUS` != 'Canceled' AND `task_info`.`MA_NGUOI_NHAN` = $id AND `user`.`MA_USER` = `user_info`.`MA_USER` AND `task`.`TASK_ID` = `task_info`.`TASK_ID` AND `task_info`.`MA_NGUOI_NHAN` = `user`.`MA_USER` ORDER BY task.DEADLINE DESC";
    $conn = getConnection();
...
function get_task($taskId){
    ...
    $sql = "SELECT user.HO_TEN, task.TIEU_DE, task.MO_TA, task.DEADLINE, task.DATE_CREATE  ,task.SUPPORT_FOLDER_PATH, task.SUBMIT_FOLDER_PATH, task.message_employee,
            task.message_tlead, task_info.COMPLETE_STATUS, task.SUBMIT_DATE
...
function update_reject_task($taskId, $employeeMessage, $submitFolder, $submitDate){
    $conn = getConnection();
    ...
    $sql = "UPDATE task
            ...
            task.message_employee = '$employeeMessage', task.SUBMIT_DATE = '$submitDate'
            WHERE TASK_ID = $taskId";
